...
async def test_get_vendor_by_id(crud, mock_vendor_collection, dummy_vendor_data):
    ...
    mock_vendor_collection.find_one.assert_called_once_with(
        {"_id": ObjectId(vendor_id), "status": {"$ne": "archived"}}
    )
...
async def test_create_vendor(crud, mock_vendor_collection, dummy_vendor_data):
    ...
    del vendor_data_to_create["_id"] # _id is generated by MongoDB
...
async def test_get_filter_fields(crud, mock_vendor_collection, dummy_vendor_data):
    ...
    mock_vendor_collection.distinct.side_effect = [
        ["USA", "IND"], # dpr_country
        ["low", "medium"], # dpr_country_risk
        ["IT", "Finance"], # industry
        ["pass", "fail"], # audit_result
    ]
    mock_vendor_collection.find.return_value.__aiter__.return_value = iter([
        ...
        {}, # Document without processing_category
        {"processing_category": "invalid_type"}, # Invalid type
        {"processing_category": None}, # None value
    ])

    result = await crud.get_filter_fields(df_id)

    # Check distinct calls
    ...
    # Check find call for processing_category
...
async def test_archive_vendor(crud, mock_vendor_collection, dummy_vendor_data):
    ...
    result = await crud.archive_vendor(vendor_id) # Await the coroutine
