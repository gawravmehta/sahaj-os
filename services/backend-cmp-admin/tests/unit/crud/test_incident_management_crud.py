...
async def test_find_duplicate(crud, mock_incident_collection, dummy_incident_data):
    ...
    mock_incident_collection.find_one.assert_called_once_with(
        {"df_id": df_id, "incident_name": incident_name, "status": "draft"}
    )
...
async def test_insert_incident(crud, mock_incident_collection, dummy_incident_data):
    ...
    del incident_data_to_insert["_id"] # _id is generated by MongoDB
...
async def test_update_incident(crud, mock_incident_collection, dummy_incident_data):
    ...
    mock_incident_collection.update_one.assert_called_once_with(
        {"_id": ObjectId(incident_id)}, {"$set": update_data}
    )
    # update_incident does not return anything, so no assertion on result
...
@pytest.mark.parametrize(
    ...
    [
        ("asc", ASCENDING),
        ("desc", DESCENDING),
        ("any_other", DESCENDING), # Default for invalid sort_order
    ],
)
...
async def test_get_filter_fields(crud, mock_incident_collection, dummy_incident_data):
    ...
    mock_incident_collection.distinct.side_effect = [
        ["Data Breach", "Security Incident"], # incident_type
        ["High", "Medium", "Low"], # incident_sensitivity
        ["draft", "published", "resolved"], # status
        ["Identification", "Containment", "Eradication"], # current_stage
    ]
...
async def test_publish_incident_failure(crud, mock_incident_collection, dummy_incident_data):
    incident_id = str(dummy_incident_data["_id"])
    mock_update_result = MagicMock(matched_count=1, modified_count=0) # No modification
    mock_incident_collection.update_one.return_value = mock_update_result
