...
def mock_collection():
    ...
    cursor.to_list = AsyncMock(return_value=[]) # Mock to_list for search and get_all
...
async def test_search_by_name(crud, mock_collection, dummy_department_data):
    ...
    mock_collection.find.assert_called_once_with(
        {"department_name": {"$regex": regex, "$options": "i"}, "df_id": df_id}
    )
...
async def test_create_department(crud, mock_collection, dummy_department_data):
    # The actual create method returns an InsertOneResult, not the dictionary directly
    # Mocking InsertOneResult
    ...
    # Remove _id from data to be created as it's generated by MongoDB
    ...
    assert result == mock_insert_one_result # Assert that the result is the InsertOneResult
