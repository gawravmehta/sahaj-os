...
QUEUES.extend(["consent_expiry_delay_queue", "consent_expiry_queue"])
...
async def declare_queues():
    ...
    try:
        ...
        for exchange_name, exchange_type in EXCHANGES.items():
            ...
            app_logger.info(f"Exchange '{exchange_name}' ({exchange_type}) declared successfully.")
        ...
        await channel.declare_queue(
            ...
            arguments={
                ...
                "x-dead-letter-routing-key": "webhook_retry",
            },
        )
        app_logger.info("Queue 'webhook_main' declared with DLQ to 'retry_exchange'.")
        webhook_exchange = await channel.get_exchange("webhook_exchange")
        webhook_main_queue = await channel.get_queue("webhook_main")
        await webhook_main_queue.bind(webhook_exchange, routing_key="webhook_main")
        app_logger.info("'webhook_main' bound to 'webhook_exchange'.")
        ...
        await channel.declare_queue(
            ...
            arguments={
                ...
                "x-dead-letter-routing-key": "webhook_main",
            },
        )
        app_logger.info("Queue 'webhook_retry' declared with TTL and DLQ back to 'webhook_exchange'.")
        retry_exchange = await channel.get_exchange("retry_exchange")
        webhook_retry_queue = await channel.get_queue("webhook_retry")
        await webhook_retry_queue.bind(retry_exchange, routing_key="webhook_retry")
        app_logger.info("'webhook_retry' bound to 'retry_exchange'.")

        await channel.declare_queue("webhook_dlq", durable=True)
        app_logger.info("Queue 'webhook_dlq' declared.")
        dlq_exchange = await channel.get_exchange("dlq_exchange")
        webhook_dlq_queue = await channel.get_queue("webhook_dlq")
        await webhook_dlq_queue.bind(dlq_exchange, routing_key="webhook_dlq")
        app_logger.info("'webhook_dlq' bound to 'dlq_exchange'.")

        await channel.declare_queue(
            "consent_events_q",
            durable=True,
            arguments={
                "x-dead-letter-exchange": "consent_retry_exchange",
                "x-dead-letter-routing-key": "consent_retry_q",
            },
        )
        app_logger.info("Queue 'consent_events_q' declared with DLQ to 'consent_retry_exchange'.")
        consent_events_exchange = await channel.get_exchange("consent_events_exchange")
        consent_events_queue = await channel.get_queue("consent_events_q")
        await consent_events_queue.bind(consent_events_exchange, routing_key="consent_events_q")
        app_logger.info("'consent_events_q' bound to 'consent_events_exchange'.")
        ...
        await channel.declare_queue(
            ...
            arguments={
                "x-message-ttl": 10000,
                "x-dead-letter-exchange": "consent_events_exchange",
                "x-dead-letter-routing-key": "consent_events_q",
            },
        )
        app_logger.info("Queue 'consent_retry_q' declared with TTL and DLQ back to 'consent_events_exchange'.")
        consent_retry_exchange = await channel.get_exchange("consent_retry_exchange")
        consent_retry_queue = await channel.get_queue("consent_retry_q")
        await consent_retry_queue.bind(consent_retry_exchange, routing_key="consent_retry_q")
        app_logger.info("'consent_retry_q' bound to 'consent_retry_exchange'.")

        await channel.declare_queue("consent_dlq", durable=True)
        app_logger.info("Queue 'consent_dlq' declared.")
        consent_dlq_exchange = await channel.get_exchange("consent_dlq_exchange")
        consent_dlq_queue = await channel.get_queue("consent_dlq")
        await consent_dlq_queue.bind(consent_dlq_exchange, routing_key="consent_dlq")
        app_logger.info("'consent_dlq' bound to 'consent_dlq_exchange'.")
        ...
        app_logger.info("Queue 'consent_expiry_delay_queue' declared with TTL and DLQ to 'consent_expiry_exchange'.")
        await channel.declare_queue("consent_expiry_queue", durable=True)
        app_logger.info("Queue 'consent_expiry_queue' declared.")
        consent_expiry_exchange = await channel.get_exchange("consent_expiry_exchange")
        consent_expiry_queue = await channel.get_queue("consent_expiry_queue")
        await consent_expiry_queue.bind(consent_expiry_exchange, routing_key="consent_expiry")
        app_logger.info("'consent_expiry_queue' bound to 'consent_expiry_exchange'.")

        await channel.declare_queue(
            "consent_processing_q",
            durable=True,
            arguments={
                "x-dead-letter-exchange": "consent_processing_retry_exchange",
                "x-dead-letter-routing-key": "consent_processing_retry_q",
            },
        )
        app_logger.info("Queue 'consent_processing_q' declared with DLQ to 'consent_processing_retry_exchange'.")
        ...
        app_logger.info("Queue 'consent_processing_retry_q' declared with TTL and DLQ back to 'consent_processing_q'.")
        consent_processing_retry_exchange = await channel.get_exchange("consent_processing_retry_exchange")
        consent_processing_retry_queue = await channel.get_queue("consent_processing_retry_q")
        await consent_processing_retry_queue.bind(consent_processing_retry_exchange, routing_key="consent_processing_retry_q")
        app_logger.info("'consent_processing_retry_q' bound to 'consent_processing_retry_exchange'.")

        await channel.declare_queue("consent_processing_dlq", durable=True)
        app_logger.info("Queue 'consent_processing_dlq' declared.")
        consent_processing_dlq_exchange = await channel.get_exchange("consent_processing_dlq_exchange")
        consent_processing_dlq_queue = await channel.get_queue("consent_processing_dlq")
        await consent_processing_dlq_queue.bind(consent_processing_dlq_exchange, routing_key="consent_processing_dlq")
        app_logger.info("'consent_processing_dlq' bound to 'consent_processing_dlq_exchange'.")
        ...
        for queue in [
            ...
            app_logger.info(f"Queue '{queue}' declared successfully.")
...
async def publish_message(
    ...
    """
    app_logger.info(f"Publishing message to queue/exchange '{queue_name}': {message}")
    ...
    try:
        ...
        elif queue_name == "webhook_retry":
            exchange_obj = await _channel.get_exchange("retry_exchange")
            routing_key_obj = "webhook_retry"
        elif queue_name == "webhook_dlq":
            exchange_obj = await _channel.get_exchange("dlq_exchange")
            routing_key_obj = "webhook_dlq"
        elif queue_name == "consent_events_q":
            exchange_obj = await _channel.get_exchange("consent_events_exchange")
            routing_key_obj = "consent_events_q"
        elif queue_name == "consent_retry_q":
            exchange_obj = await _channel.get_exchange("consent_retry_exchange")
            routing_key_obj = "consent_retry_q"
        elif queue_name == "consent_dlq":
            exchange_obj = await _channel.get_exchange("consent_dlq_exchange")
            routing_key_obj = "consent_dlq"
        ...
        elif queue_name == "consent_expiry_queue":
            exchange_obj = await _channel.get_exchange("consent_expiry_exchange")
            routing_key_obj = "consent_expiry"
